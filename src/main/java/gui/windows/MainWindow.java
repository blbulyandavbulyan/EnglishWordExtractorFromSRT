...
import wordprocessing.wordextractor.WordExtractor;
                        ...
                        JTable wordsTable = new Callable<JTable>(){
                            ...
                            public JTable call(){
                                ...
                                if(needTranslate = true){
                                    ...
                                    WordInfo[] wordInfos = wordInfoGenerator.generateWordInfos(minimalWordInfos);
                                    for (int j = 0; j < wordInfos.length; j++) {

                                        wordInfos[j] = new WordInfo("test", 23 + j, new String[]{"тест", "тест2"}, new String[]{"сущ", "прил"});
                                    }
                                    return fillAndCreateTable(wordInfos);
                                    //fixme
                                }
                                else {
                                    return fillAndCreateTable(minimalWordInfos);
                                }
                            }
                        }.call();
